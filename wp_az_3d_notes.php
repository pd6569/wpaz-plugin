...
class wp_az_3d_notes {
	...
	public function enqueue() {
		...
		if (wp_az_show_plugin_layout()){
			...
			// App JS bundle
			wp_enqueue_script('wp_az_3d_notes_main', plugins_url('dist/bundle.js', __FILE__), array('jquery'), '0.5.6', true);
...
/*			// ES6
			...
			wp_enqueue_script('wp_az_module_base_class', plugins_url('js/BaseModule.js', __FILE__), array('jquery'), '1.0', true);
	...
	public function upload_snapshot() {
		...
		if ($attach_id) {
			$attach_src_thumbnail = wp_get_attachment_image_src($attach_id, 'thumbnail')[0];
			...
			$attach_src_full = wp_get_attachment_image_src($attach_id, 'full')[0];
		...
		wp_send_json(array(
			...
			'attachment_src_full'       => $attach_src_full
		));
